...
export async function handleListStocks(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    if (portfolio.stocks.length === 0) {
      await interaction.editReply({ content: 'Bạn chưa có cổ phiếu nào trong danh mục.' });
      return;
    }
    ...
    await interaction.editReply({ content: '❌ Đã xảy ra lỗi khi hiển thị danh mục cổ phiếu.' });
  }
} 